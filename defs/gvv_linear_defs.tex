...
\begin{abstract}
This manual provides a simple introduction to linear forms like lines and planes,
based on the NCERT textbooks from Class 6-12.  
\end{abstract}
...
which can be simplified to obtain 
	...
	d^2\brak{\lambda } =\lambda^2 \norm{\vec{m}}^2+2\lambda \vec{m}^{\top}\brak{\vec{A} 
		...
		Now, $d\brak{\lambda}$ is smallest when the above quadratic equation has a single root, i.e. 
		\begin{align}
			\lambda = -\frac{\vec{m}^{\top}\brak{\vec{A} 
			-\vec{P}}}
			{ \norm{\vec{m}}^2}
			\label{dist_3d_lam}
		\end{align}
